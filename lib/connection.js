    ...
    if (connection === 'close') {
      ...
      switch (reason) {
        ...
        case DisconnectReason.connectionReplaced:
          console.log(
            chalk.redBright(`Connection Replaced, Another New Session Opened, Please Restart Bot`),
          );
          logWithTime(
            'System',
        ...
        case DisconnectReason.loggedOut:
          console.log(
            chalk.redBright(
              `Perangkat Terkeluar, Hapus Folder Session dan Lalukan Scan/Pairing Ulang`,
            ),
          );
          logWithTime(
            'System',
        ...
        default:
          ...
          if (reason === 403) {
            ...
            if (error403Timestamps.length > 3) {
              console.log(
                chalk.bgRed(
                  '[BLOCKED] Terlalu banyak error 403 dalam 1 menit. Stop reconnect sementara.',
                ),
              );
      ...
      if (isRestart) {
        if (isSession) {
          await sock.sendMessage(isRestart, {
            text: '_Bot Berhasil di restart_',
          });
        }
      } else if (isSession) {
      ...
      try {
        ...
        console.log(
          chalk.redBright(`Error dalam menjalan updateSocket atau waktuSholat : ${error.message}`),
        );
      }
